...
class ST3215_Comm {
public:
  enum ST3215_MSG_INDEX { HEADER_H = 0, HEADER_L, ID, LENGTH, INSTRUCTION, PARAM_1, PARAM_2, CRC };

  // Mode List
  enum MODE { SERVO = 0, WHEEL };
  ...
  virtual void createMsg(uint8_t id, uint8_t instruction, uint8_t param1, uint8_t param2, uint8_t *msg,
                         uint8_t msg_len);
  virtual void createWord(uint8_t id, uint8_t instruction, uint8_t param1, uint8_t param2, uint8_t param3, uint8_t *msg,
                          uint8_t msg_len);
};
#endif
