...
ReadBack_Status ST3215_Comm::readWord(HardwareSerial *serial, int16_t &recv) {
  ...
  while (serial->available()) {
    ...
    if (buffer[ST3215_MSG_INDEX::HEADER_H] != 0xFF) {
      ctn = 0;
    } // Not correct header
    else if (ctn < MSG_SIZE) {
    } // Not enough data
    else {
#ifdef DEBUG_MSG
      Serial.print("WOW -> ");
...
void ST3215_Comm::createMsg(uint8_t id, uint8_t instruction, uint8_t param1, uint8_t param2, uint8_t *msg,
                            uint8_t msg_len) {
...
void ST3215_Comm::createWord(uint8_t id, uint8_t instruction, uint8_t param1, uint8_t param2, uint8_t param3,
