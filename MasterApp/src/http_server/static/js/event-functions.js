...
async function setupPageElements() {
  ...
  ].map((i) => `cage${window.row - 1}x${i.toString(14).padStart(4, "0")}`);
...
async function setupActionExecution() {
  ...
  executeButton.addEventListener("click", function () {
    const selectedCages = Array.from(cageCheckboxes)
      .filter((chk) => chk.checked)
      .map((chk) => chk.id);
    const selectedActions = Array.from(actionCheckboxes)
      .filter((chk) => chk.checked)
      .map((chk) => post_request_dict[chk.value]);
...
class CageStatusUpdater {
  ...
  SensorIndicators() {
    Object.keys(this.data).forEach((cage) => {
      ...
      const sensors = this.data[cage].sensors_values.replace(/[()]/g, "").split(",").map(Number);
  ...
  SensorIndicator(elementId, sensors, index) {
    ...
    if (sensorCell) {
      sensorCell.classList.remove("indicator-sensor", "indicator-not-triggered");
  ...
  GearStatuses() {
    Object.keys(this.data).forEach((cage) => {
      this.GearColor(cage, "star_wheel", this.data[cage].star_wheel_status);
...
class PotSorterStatusUpdater {
  ...
  PotSorterMode() {
    ...
    if (this.a1Data.connected === "True" && this.a1Data.running === "True" && this.a1Data.buff_out === "True") {
      ...
      (this.a1Data.connected === "True" && this.a1Data.running === "False") ||
      this.a1Data.connected === "False"
    ) {
...
class PotDispenserStatusUpdater {
  ...
  PotDispenserMode() {
    ...
    if (
      ...
      (this.a3Data.connected === "True" && this.a3Data.running === "False") ||
      this.a3Data.connected === "False"
    ) {
...
class ChimneySorterStatusUpdater {
  ...
  ChimneySorterMode() {
    ...
    if (this.c1Data.connected === "True" && this.c1Data.running === "True" && this.c1Data.buff_out === "True") {
      ...
      (this.c1Data.connected === "True" && this.c1Data.running === "False") ||
      this.c1Data.connected === "False"
    ) {
...
class ChimneyPlacerStatusUpdater {
  ...
  ChimneyPlacerMode() {
    const chimenySorterIndicator = document.querySelector(".chimney-sorter-mode");
    if (
      this.c3Data.connected === "True" &&
      this.c3Data.running === "True" &&
      this.c3Data.pot_sensor === "True" &&
      this.c3Data.chimney_sensor === "True"
    ) {
      chimenySorterIndicator.textContent = "Running";
      chimenySorterIndicator.style.backgroundColor = "#4caf50"; // Green for running
    } else if (
      this.c3Data.connected === "True" &&
      this.c3Data.running === "True" &&
      (this.c3Data.pot_sensor === "False" || this.c3Data.chimney_sensor === "False")
    ) {
      chimenySorterIndicator.textContent = "Waiting";
      chimenySorterIndicator.style.backgroundColor = "orange"; // Yellow for waiting
    } else if (
      (this.c3Data.connected === "True" && this.c3Data.running === "False") ||
      this.c3Data.connected === "False"
    ) {
      chimenySorterIndicator.textContent = "Idle";
      chimenySorterIndicator.style.backgroundColor = "grey"; // Grey for idle
    }
...
class Controller_1A_1C {
  ...
  setupEventListeners() {
    ...
    if (
      ...
      raiseNozzleButton &&
      lowerNozzleButton &&
      clearErrorSW2Button2 &&
      homeSW2Button2 &&
      clearErrorSW2Button3 &&
      homeSW2Button3
    ) {
