...
class ComputerVision:
        ...
        for index in range(head_num):
            ...
            for h in range(map_size[index][0]):
                for w in range(map_size[index][1]):
                    ...
                    if 1 == class_num:
                        cls_max = self.sigmoid(cls[0 * map_size[index][0] * map_size[index][1] + h * map_size[index][1] + w])
                        cls_index = 0
                    else:
    ...
    def pre_process(self, img, input_height, input_width):
    ...
    def prepare_inference_data(self,outputs, img_h, img_w):
        """
