...
# =================== Setup Cooling Fan ================== #
sudo cp ./control_fan.sh  /usr/local/bin/. && sudo chmod +x /usr/local/bin/control_fan.sh
sudo cp ./cooling_fan.service  /etc/systemd/system/. && sudo systemctl daemon-reload && systemctl enable cooling_fan.service
echo -e "${green}[SUCESS]>> Configured Cooling Fan${nc}"
