...
void Communication::update()
{
  ...
  // Check any message
  if (!pri_isMsgValid()) return;
  /* ############################################################################################ */
  /*                                         MAIN HANDLER                                         */
  /* ############################################################################################ */
  switch (m_stMsg.header)
  {
  case HEADER_ACTION: pri_actionMsgHandler(); break;
  case HEADER_SENSE: pri_senseMsgHandler(); break;
  }
...
{
  if (m_serial == nullptr) return;        // Sanity check
  if (m_serial->available() == 0) return; // No data
  m_msg[m_counter++] = m_serial->read();
  if ((m_counter > 0) && !pri_isHeaderCorrect()) m_counter = 0;
}

bool Communication::pri_isHeaderCorrect() const
{
  return ((m_msg[0] == HEADER_ACTION) ||
          (m_msg[0] == HEADER_SENSE));
}

bool Communication::pri_isMsgValid()
{
  if (m_counter < MSG_SIZE) return false;
  m_counter = 0;
...
void Communication::pri_unloaderHomeHandler()
{
  ...
  ReadBack_Status status = m_unloader->homingByLoad();
  replyReadbackStatus(status);
}
...
void Communication::pri_starWheelSensorHandler()
{
  switch (m_stMsg.action)
  {
  case SENSE_ERROR_STATUS: pri_starWheelErrorHandler(); break;
  }
...
void Communication::pri_unloaderSensorHandler()
{
  switch (m_stMsg.action)
  {
  case SENSE_ERROR_STATUS: pri_unloaderErrorHandler(); break;
  }
}
